...
// TestNewTrackerConfig verifies that the default configuration is correctly created.
func TestNewTrackerConfig(t *testing.T) {
	...
	if config.ConsumerGroup != DefaultConsumerGroup {
		t.Errorf("Expected ConsumerGroup to be %s, got %s", DefaultConsumerGroup, config.ConsumerGroup)
	}
	if config.Topic != DefaultTopic {
		t.Errorf("Expected Topic to be %s, got %s", DefaultTopic, config.Topic)
	}
	if config.MaxErrors != TrackerMaxConsecutiveErrors {
		t.Errorf("Expected MaxErrors to be %d, got %d", TrackerMaxConsecutiveErrors, config.MaxErrors)
	}
}

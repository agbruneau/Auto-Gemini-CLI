...
async function getData(type, filename) {
    ...
    if (window.EMBEDDED_DATA && window.EMBEDDED_DATA[type]) {
        return window.EMBEDDED_DATA[type];
    }
...
async function loadComplexityData() {
    try {
        const text = await getData('complexity', 'complexity_comparison.csv');
        ...
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: 'Iterative (ns)',
                        data: data.map(row => parseFloat(row.iterative_ns) || 0),
                        borderColor: '#f59e0b',
                    ...
                    {
                        label: 'Matrix (ns)',
                        data: data.map(row => parseFloat(row.matrix_ns) || 0),
                        borderColor: '#10b981',
...
async function loadBinetData() {
    try {
        const text = await getData('binet', 'binet_accuracy.csv');
        const data = parseCSV(text);
                ...
                datasets: [
                    {
                        ...
                        data: data.map(row => {
                            const val = parseFloat(row.rel_error);
...
async function loadGoldenData() {
    try {
        const text = await getData('golden', 'golden_ratio_convergence.csv');
        const data = parseCSV(text);
        ...
        document.getElementById('goldenData').innerHTML = 
            `<strong>Convergence:</strong> Error from phi at n=50: ${parseFloat(lastError).toExponential(4)}`;
    } catch (error) {
